...
export default function CourseDetail({ course: propCourse, onStartLearning }) {
  ...
  const siteTitle = config.site_title || 'Ekko Academy';
  const pageTitle = `${course.title} - ${siteTitle}`;
  const pageDesc = course.description || config.site_description;
              ...
              <div className="bg-slate-50 rounded-2xl p-8 mb-8">
                <div className="flex items-baseline gap-2 mb-4">
                  ...
                  {course.original_price > course.price && (
                      <span className="text-xl text-slate-400 line-through">¥{course.original_price}</span>
                  )}
                  {course.original_price > course.price && (
                      <span className="ml-2 px-2 py-1 bg-red-100 text-red-600 text-xs font-bold rounded-full">
                          限时 {Math.round((course.price / course.original_price) * 10)} 折
                      </span>
                  )}
                </div>
